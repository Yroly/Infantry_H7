...
LegClass::LegClass(){
	...
	Leg_set.L0_set = 0.23;//腿长初值
	K_filter_gyro=0.6;
}
void VmcClass::K_Matching(){//更新k增益值
	Zjie();//先正解出L0的值再更新K值
	Support_Force_resolving();//支持力解算
}	
/**
 * @brief: 运动学逆解
 * @author: Dandelion
 * @param {float} xc五连杆末端坐标
 * @param {float} yc五连杆目标末端坐标可以解算出所需关节角度
 * @return {*}
 */
void VmcClass::Njie(const float xc, const float yc){
    ...
    // nije_5(&angle1, (void *)0, x, y, l1, l6, l3, l4, l5); //利用l1和l6计算c1;
    m = l1 * cos(angle1);
    n = l1 * sin(angle1);
    b = 0;
    // x1 = l2 / l6 * ((x - m) * cos(b) - (y - n) * sin(b)) + m;
    // y1 = l2 / l6 * ((x - m) * sin(b) + (y - n) * cos(b)) + n;得到闭链五连杆端点坐标
    x1 = ((xc - m) * cos(b) - (yc - n) * sin(b)) + m;
    y1 = ((xc - m) * sin(b) + (yc - n) * cos(b)) + n; //得到闭链五连杆端点坐标
 ...
 * @brief: 运动学正解
 * @author: Dandelion
 * @param {float} angle1前关节角度
 * @param {float} angle4后关节角度
 * @param {float} pitch %rad上层机构偏移
 * @return {*}
 */
void VmcClass::Zjie(){
	  xb = l1 * cos(angle1);
...
/**
 * @brief: VMC_Positive虚拟力算法正解
 * @author: Dandelion
 * @param {float} TP_H_Q 
 * @param {float} F 
 * @param {float} Tp 杆所受力矩向左
 * @return {*}
 */
float TP_limit=0.5;
...
void VmcClass::KF_Feedback_Update()
{
   ...
   Wheel_Speed_Forecast     =Wheel_Speed_fdb+WHEEL_RADIUS*(-Leg_Ctrl.Leg_fdb.Pitch_Gyro_fdb-WBR.Tilt_angle_0.dot_now)+WBR.L0_fdb.now*WBR.Tilt_angle_0.dot_now*cos(WBR.Tilt_angle_0.now)+WBR.L0_fdb.dot_now*sin(WBR.Tilt_angle_0.now);//�ٶȹ۲�ֵ
