...
Mahony::Mahony(float dt, float kp, float ki)
    : dt_(dt), two_kp_(2 * kp), two_ki_(2 * ki), inited_(false),
      integral_fbx_(0.0f), integral_fby_(0.0f), integral_fbz_(0.0f) {}

void Mahony::update(const float acc[3], const float gyro[3]) {
  update(acc[0], acc[1], acc[2], gyro[0], gyro[1], gyro[2]);
}

void Mahony::update(float ax, float ay, float az, float wx, float wy,
                    float wz) {
  if (!inited_) {
    inited_ = true;
    init(ax, ay, az);
  ...
  float norm = 1.0f / sqrt(ax * ax + ay * ay + az * az);
  ax *= norm;
  ay *= norm;
  az *= norm;
  ...
  float q0 =
      this->q[0] + (-this->q[1] * wx - this->q[2] * wy - this->q[3] * wz);
  ...
  // 这个四元数是归一化之后的四元数,
  // 而且我检验了,这个四元数Q_(W <- G )=
  // {q[0],q[1],q[2],q[3]}是标量在前形式,后边虚部所表示的向量是旋转轴在地面系W下的坐标
  // 这个四元数的意义是
  // 1.将同一个向量在云台系下的坐标转换到地面系下的坐标即 v_world = Q * v_gimbal
  // * Q_conjugate
  ...
  this->yaw =
      atan2(2.0f * (this->q[0] * this->q[3] + this->q[1] * this->q[2]),
            1.0f - 2.0f * (this->q[2] * this->q[2] + this->q[3] * this->q[3]));
  this->pitch =
      asin(2.0f * (this->q[0] * this->q[2] - this->q[1] * this->q[3]));
  this->roll =
      atan2(2.0f * (this->q[0] * this->q[1] + this->q[2] * this->q[3]),
            1.0f - 2.0f * (this->q[1] * this->q[1] + this->q[2] * this->q[2]));

  culculate_yaw_pitch_roll_rates(gx, gy, gz, this->roll, this->pitch,
                                 this->yaw);
  // 调用函数计算对应欧拉角的微分
}

void Mahony::culculate_yaw_pitch_roll_rates(float wx, float wy, float wz,
                                            float roll, float pitch,
                                            float yaw) {
  this->vroll = wx + wy * sin(roll) * tan(pitch) + wz * cos(roll) * tan(pitch);
  this->vpitch = wy * cos(roll) - wz * sin(roll);
  this->vyaw = wy * sin(roll) / cos(pitch) + wz * cos(roll) / cos(pitch);
}
...
void Mahony::init(float ax, float ay, float az) {
  ...
  float pitch0 = atan2(-ax, az);
  float roll0 = atan2(ay, az);
  float yaw0 = 0.0f;
  // 需要注意的是:上电后在确定底盘相对于地面的初始yaw值的时候
  // 要在mahony的init阶段直接将yaw_relative_angle赋值给底盘系imu的yaw0(正负自己确定)!!!
  float cr2 = cos(roll0 * 0.5f);
  float cp2 = cos(pitch0 * 0.5f);
  float cy2 = cos(yaw0 * 0.5f);
  float sr2 = sin(roll0 * 0.5f);
  float sp2 = sin(pitch0 * 0.5f);
  float sy2 = sin(yaw0 * 0.5f);
